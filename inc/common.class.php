...
class PluginMreportingCommon extends CommonDBTM
{
        ...
        if ($handle = opendir($inc_dir)) {
            while (false !== ($entry = readdir($handle))) {
                if ($entry != '.' && $entry != '..') {
                    ...
                    if (
                        preg_match(
                            "/class\s(.+)Extends PluginMreporting.*Baseclass/i",
                            $fcontent,
                            $matches,
                        )
                    ) {
        ...
        foreach ($reports as $classname => $report) {
            ...
            if (isset($graphs[$classname]) && $count > 0) {
                ...
                echo "<a href='#' onClick=\"var w = window.open('popup.php?classname=" . htmlspecialchars($classname) . "' ,'glpipopup', " .
                  "'height=" . htmlspecialchars($height) . ", width=1000, top=100, left=100, scrollbars=yes'); w.focus();\">";
                echo 'ODT</a>';
                echo '</div>';
            } else {
    ...
    */
    public function showExportForm($opt)
    {
        ...
        if ($classname) {
            ...
            echo "<a onclick= \"if ( markCheckboxes('" . htmlspecialchars($formname) . "') ) return false;\" href='#'>" . __('Check all') . '</a></td>';
            echo '<td>/</td>';
            echo "<td class='center' style='white-space:nowrap;'>";
            echo "<a onclick= \"if ( unMarkCheckboxes('" . htmlspecialchars($formname) . "') ) return false;\" href='#'>" . __('Uncheck all') . '</a></td>';
    ...
    */
    public static function showGraphDatas(
        ...
        echo "<div align='center' style='" . htmlspecialchars($visibility) . "' id='view_datas" . htmlspecialchars($rand) . "'>";
        echo "<table class='tab_cadre' width='90%'>";
    ...
    */
    public function export($opt)
    {
        ...
        if (isset($opt['classname'])) {
                    ...
                    foreach ($report['functions'] as $func) {
                        foreach ($opt['check'] as $do => $to) {
                            if ($do == $func['function'] . $classname) {
                                ...
                                if (
                                    isset($LANG['plugin_mreporting'][$func['short_classname']][$func['function']]['desc'])
                                    && isset($opt['date1'])
                                    && isset($opt['date2'])
                                ) {
                                    $des_func .= ' - ';
            ...
            if (
                isset($LANG['plugin_mreporting'][$opt['short_classname']][$opt['f_name']]['desc'])
     ...
     * @return integer|string selector
     */
     ...
     * @param  string $date1    date of start
     * @param  string $date2    date of ending
    ...
    public static function saveSelectors($graphname, $config = [])
    {
        ...
        if (isset($config['randname'])) {
            if (
                isset($_REQUEST['date1' . $config['randname']])
            ...
            if (
                isset($_REQUEST['date2' . $config['randname']])
     ...
     * @param  array $raw_tree the tree array.
